...
const readStream = node_fs_1.default.createReadStream(node_path_1.default.join(__dirname, '../data/products.csv'));
const writeStream = node_fs_1.default.createWriteStream(node_path_1.default.join(__dirname, '../data/clean_products.csv'));
const transformer = (0, csv_1.transform)((data) => {
    ...
    for (let productObj of jsonParsedData) {
        ...
        result.push(`${id},"${name}","${slogan}","${description}","${category}",${default_price}\n`);
    }
    return result.join('\n');
});
readStream
